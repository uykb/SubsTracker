      ...
      <form id="configForm" class="space-y-8">
        ...
        <div class="border-b border-gray-200 pb-6">
          ...
          <div class="mb-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <label class="inline-flex items-center">
                ...
                <span class="ml-2 text-sm text-gray-700">Bark通知</span>
              </label>
            </div>
            <div class="mt-2 flex flex-wrap gap-4">
              ...
              <a href="https://bark.day.app/" target="_blank" class="text-indigo-600 hover:text-indigo-800 text-sm">
                <i class="fas fa-external-link-alt ml-1"></i> Bark官网
              </a>
            </div>
          </div>
          ...
          <div id="barkConfig" class="config-section">
            <h4 class="text-md font-medium text-gray-900 mb-3">Bark通知 配置</h4>
            <div class="grid grid-cols-1 gap-4 mb-4">
              <div>
                <label for="barkDeviceKey" class="block text-sm font-medium text-gray-700">设备密钥</label>
                <input type="text" id="barkDeviceKey" placeholder="your-device-key" class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
                <p class="mt-1 text-sm text-gray-500">从 Bark App 中获取的设备密钥</p>
              </div>
              <div>
                <label for="barkServer" class="block text-sm font-medium text-gray-700">服务器地址 (可选)</label>
                ...
                <p class="mt-1 text-sm text-gray-500">自定义服务器地址，留空使用默认服务器</p>
              </div>
              <div>
                <label for="barkSound" class="block text-sm font-medium text-gray-700">通知声音 (可选)</label>
                <select id="barkSound" class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
                ...
                <p class="mt-1 text-sm text-gray-500">选择通知声音，留空使用默认声音</p>
              </div>
              <div>
                <label for="barkIcon" class="block text-sm font-medium text-gray-700">通知图标 (可选)</label>
                ...
                <p class="mt-1 text-sm text-gray-500">自定义通知图标URL</p>
              </div>
              <div>
                <label for="barkGroup" class="block text-sm font-medium text-gray-700">通知分组 (可选)</label>
                <input type="text" id="barkGroup" placeholder="订阅提醒" class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
                <p class="mt-1 text-sm text-gray-500">通知分组名称，用于归类通知</p>
              </div>
              <div>
                <label for="barkUrl" class="block text-sm font-medium text-gray-700">点击跳转URL (可选)</label>
                <input type="url" id="barkUrl" placeholder="https://example.com" class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
                <p class="mt-1 text-sm text-gray-500">点击通知时跳转的URL</p>
              </div>
            </div>
            <div class="flex justify-end">
              <button type="button" id="testBarkBtn" class="btn-secondary text-white px-4 py-2 rounded-md text-sm font-medium">
                <i class="fas fa-paper-plane mr-2"></i>测试 Bark通知
              </button>
            </div>
          </div>
        </div>
  ...
  <script>
    ...
    document.getElementById('configForm').addEventListener('submit', async (e) => {
      ...
      const config = {
        ...
        BARK_SERVER: document.getElementById('barkServer').value.trim(),
        BARK_SOUND: document.getElementById('barkSound').value.trim(),
        BARK_ICON: document.getElementById('barkIcon').value.trim(),
        BARK_GROUP: document.getElementById('barkGroup').value.trim(),
    ...
    async function testNotification(type) {
      ...
      if (type === 'telegram') {
        ...
        config.BARK_SERVER = document.getElementById('barkServer').value.trim();
...
async function sendBarkNotification(title, content, config) {
  try {
    ...
    if (config.BARK_ICON) {
      params.append('icon', config.BARK_ICON);
    }
    if (config.BARK_GROUP) {
      params.append('group', config.BARK_GROUP);
    }
    if (config.BARK_URL) {
      params.append('url', config.BARK_URL);
    }
